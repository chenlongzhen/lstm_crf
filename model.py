...
class Model(object):
    def __init__(self, config):
        ...
        with tf.variable_scope("optimizer"):
            ...
            # apply grad clip to avoid gradient explosion 做了一个梯度截断
    ...
    def biLSTM_layer(self, model_inputs, lstm_dim, lengths, name=None):
        ...
        with tf.variable_scope("char_BiLSTM" if not name else name):
            ...
            outputs, final_states = tf.nn.bidirectional_dynamic_rnn(
                ...
                model_inputs,
                dtype=tf.float32,
                sequence_length=lengths)#lengths句子的实际长度
    ...
    def IDCNN_layer(self, model_inputs, 
        ...
        """
        model_inputs = tf.expand_dims(model_inputs, 1)#[batch,1,length,dim]
        reuse = False
        if self.dropout == 1.0:
            reuse = True
        with tf.variable_scope("idcnn" if not name else name):
            ...
            """
            layerInput = tf.nn.conv2d(model_inputs,#[batch,1,length,100]
                                      filter_weights,
                                      strides=[1, 1, 1, 1],
                                      padding="SAME",
